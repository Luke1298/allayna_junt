...
def setplotfg(fgno=1, outdir='_output'):
    ...
    if fgno>0:
        ...
        #fgdata.clines = arange(-200,200,10)  #turned off contours for now

        # For plot of surface eta each frame:
        fgdata.eta_show = True #change this
...
#                0.4*etamax:[0,1,0], etamax:[1,0.2,0.2]})
        fgdata.inundated_cmap =  colormaps.make_colormap({0:[0,0,0],\
                0.2*etamax:[0,0,0], 0.4*etamax:[0,0,0.25],1.01*etamax:[0,0,0], \
                0.6*etamax:[0,0.5,0.75], 0.8*etamax:[1,1,0.75], etamax:[1,1,1]})
        ...
        fgdata.seafloor_add_colorbar = False
